...
@section('title', 'Edit Logbook Mingguan')
@section('content')
    ...
    <x-cards.regular-card heading="Edit Logbook Mingguan">
        <form action="{{ route('aktivitas.logbook.mingguan.update', ['id' => $dataLogbookMingguan->id]) }}" method="POST" autocomplete="off" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="d-flex flex-row">
                <x-inputs.textfield class="flex-grow-1 mr-3" label="Judul" name="judul" isRequired="true" value="{{ old('judul') ?? ($dataLogbookMingguan->judul ?? null) }}" />
            </div>
            <div class="d-flex flex-row">
                <x-inputs.textfield class="flex-grow-1 mr-3" label="Deskripsi" name="deskripsi" value="{{ old('deskripsi') ?? ($dataLogbookMingguan->deskripsi ?? null) }}" />
            </div>
            <div class="d-flex flex-row">
                <!-- <x-inputs.textfield class="flex-grow-1 mr-3" label="URL Youtube" name="link_youtube" value="{{ old('link_youtube') ?? ($dataLogbookMingguan->link_youtube ?? null) }}"/> -->
            </div>
            <div class="d-flex flex-row">
                ...
                <div class="flex-grow-1 mr-3 mb-3">
                    ...
                    <select class="form-control" name="minggu" id="minggu_ke">
                        ...
                        @for ($i = 1; $i < 26; $i++)
                            <option value="{{ $i }}" {{ (old('minggu') ?? ($dataLogbookMingguan->minggu ?? null)) == $i ? 'selected' : '' }}>Minggu ke - {{ $i }}</option>
                        @endfor
                    </select>
                    @error('minggu')
            ...
            <div class="d-flex flex-row">
                ...
                <div class="flex-grow-1 mr-3 mb-3">
                    ...
                    <input type="date" class="form-control" name="tanggal" id="tanggal" value="{{ old('tanggal') ?? (($dataLogbookMingguan->tanggal ? \Carbon\Carbon::parse($dataLogbookMingguan->tanggal)->format('Y-m-d') : null) ?? null) }}">
                    @error('tanggal')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
