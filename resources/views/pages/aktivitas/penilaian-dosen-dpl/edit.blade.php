...
@section('title', 'Edit Penilaian Dosen')
@section('content')
    ...
    <x-cards.regular-card heading="Edit Penilaian Dosen">
        <form action="{{ route('aktivitas.penilaian_dosen_dpl.update', ['id' => $dataPenilaianDosen->id]) }}" method="POST" autocomplete="off" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select class="form-control" name="registrasi_mbkm_id" id="id_validasi_registrasi" onchange="getIdRegistrasiData(this); getLogbookMingguan(this); getLaporanAkhirMahasiswa(this)">
                    ...
                    @foreach ($listRegistrasiMbkm as $reg)
                        <option value="{{ $reg->id }}" {{ (old('registrasi_mbkm_id') ?? ($dataPenilaianDosen->registrasi_mbkm_id ?? null)) == $reg->id ? 'selected' : '' }}>
...
@section('js')
    ...
    @if (old('registrasi_mbkm_id') ?? $dataPenilaianDosen->registrasi_mbkm_id)
        <script>
            $('#btn-submit').removeAttr('disabled')
            $.ajax({
                url: "{{ url('dashboard/aktivitas/registrasi-mbkm/detail') . '/' . (old('registrasi_mbkm_id') ?? $dataPenilaianDosen->registrasi_mbkm_id) }}",
                // accepts: 'application/json',
                success: function(resp) {
                    console.log(resp);
                    if (resp.isOk) {
            ...
            $.ajax({
                ...
                // accepts: 'application/json',
                data: {
                    registrasi_mbkm_id: "{{ old('registrasi_mbkm_id') ?? $dataPenilaianDosen->registrasi_mbkm_id }}"
                },
                success: function(resp) {
                    console.log(resp);
                    console.log('success get data... logbook mingguan');
            ...
            $.ajax({
                url: "{{ url('dashboard/aktivitas/laporan-akhir/mahasiswa/get-by-registrasi-id') . '/' . (old('registrasi_mbkm_id') ?? $dataPenilaianDosen->registrasi_mbkm_id) }}",
                // accepts: 'application/json',
                // data: function(d){
                //     d.registrasi_mbkm_id = id
                // },
